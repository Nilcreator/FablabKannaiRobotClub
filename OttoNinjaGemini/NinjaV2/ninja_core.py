...
GOOGLE_API_KEY = "Input your Gemini API here!"  # <----------- input your API key
...
def cleanup_all():
    ...
    # !! Declare intent to use these global variables !!
    global keep_distance_checking, movement_thread, distance_check_thread
    global is_continuous_moving, hardware_initialized, buzzer_pwm
    play_robot_sound("sleepy") # Play a sleepy sound
    ...
    if hardware_initialized:
        ...
        # 3. Stop Buzzer (Now correctly uses the global buzzer_pwm)
        if buzzer_pwm: # Check if it was successfully initialized
            print("Stopping buzzer...")
            try:
                buzzer_pwm.stop()
            except Exception as buzz_e:
                print(f"Error stopping buzzer: {buzz_e}")
            # Don't set buzzer_pwm to None here, cleanup should handle releasing resources
        else:
            print("Buzzer PWM object not found or not initialized, skipping stop.")
        ...
        print("Skipping hardware cleanup as initialization may have failed.")

    # Reset flags after cleanup attempts
    hardware_initialized = False
    buzzer_pwm = None # Clear reference after cleanup/stop attempt
