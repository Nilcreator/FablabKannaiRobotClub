...
if len(my_api_key) > 5:
    ...
    os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/path/to/your/service_account_key.json" #Replace /path/to/your/service_account_key.json with the actual path to your json
    genai.configure(transport='rest', location='us-central1')  # SPECIFY REGION HERE
    #genai.configure()
...
def main():
    ...
    while True:     
        ...
        with mic as source:            
            ...
            try: 
                #gled.on()
                print("Speak to enter your prompt or question...")                
                ...
                if len(text)>0:
                    print(f"You: {text}\n " )
                    print(text.lower().split("google")[1])
                else:
                    print(f"Unable to recognize your speech.\n " )
                    continue  
                
                #gled.off()                 
                # AI is in sleeping mode
                if sleeping == True:
                    # User can start the conversation with the wake word "Google"
                    # This word can be chagned below. 
                    if "google" in text.lower():
                        request = text.lower().split("google")[1]
                        #print("Chat start:"+text.lower().split("google")[1])
